...
class MajorityLearner(Learner):
    ...
    def train(self, dataset):
        ...
        self.most_popular = dataset.values[target][0]
        maxWeight = 0
        for v in dataset.values[target]:
            weightSum = 0
            for example in dataset.examples:
                if example.attrs[target] == v:
                    weightSum += example.weight
            if weightSum > maxWeight:
                maxWeight = weightSum
                self.most_popular = v # want the value that's most popular
...
class DecisionTreeLearner(Learner):
    ...
    def train(self, dataset):
        ...
        self.dt = self.decision_tree_learning(dataset.examples, dataset.inputs, 0, dataset.max_depth)

    def decision_tree_learning(self, examples, attrs, depth, maxDepth, default=None):
        ...
        if len(examples) == 0:
            ...
            # for cutting off the tree when maximum depth has been reached 
            return DecisionTree(DecisionTree.LEAF, classification=self.majority_value(examples))
        else:
            ...
            for (v, examples_i) in self.split_by(best, examples):
                subtree = self.decision_tree_learning(examples_i,
                  removeall(best, attrs), depth + 1, maxDepth, self.majority_value(examples))
                subtree.depth += 1
                tree.add(v, subtree)
            return tree
    ...
    def count(self, attr, val, examples):
        # updated to account for examples' weights 
        sum_weights = 0
    ...
    def add_weights (self, examples, attr=-1, val=-1):
        """ 
            Adds weights of the examples with attribute = val
        """
        total = 0.0
        if attr!=-1:
            for example in examples:
    ...
    def information_gain(self, attr, examples):
        """
            Updated information gain criterion to account for example weights 
        """
        def I(examples):
            target = self.dataset.target
            weightSumList = []
            for v in self.dataset.values[target]:
                weightSumList.append(self.add_weights(examples, target, v))
        ...
        for (v, examples_i) in self.split_by(attr, examples):
            weightSumAttr = self.add_weights(examples_i, attr, v)
